...
ART = ROOT / "docs" / "art"
SCHEMA = "behavior-transform-io.repo-art/v1"
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "behavior-transform-io", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('behavior-transform-io') % 100000:05d}"
